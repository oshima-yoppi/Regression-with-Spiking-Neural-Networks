...
class Conv4Regression(torch.nn.Module):
    def __init__(self, num_time=20, l_tau=0.8, soft=False, rec=False, forget=False, dual=False, power=False, gpu=True,
        ...
        n3 = 1
        # Encoder layers
        self.l1 = snu_layer.Conv_SNU(in_channels=c1, out_channels=c2, kernel_size=3, padding=1, l_tau=l_tau, soft=soft, rec=self.rec, forget=self.forget, dual=self.dual, gpu=gpu)
        self.l2 = snu_layer.Conv_SNU(in_channels=c2, out_channels=c3, kernel_size=3, padding=1, l_tau=l_tau, soft=soft, rec=self.rec, forget=self.forget, dual=self.dual, gpu=gpu)
        self.l3 = snu_layer.Conv_SNU(in_channels=c3, out_channels=c4, kernel_size=3, padding=1, l_tau=l_tau, soft=soft, rec=self.rec, forget=self.forget, dual=self.dual, gpu=gpu)
        self.l4 = snu_layer.Conv_SNU(in_channels=c4, out_channels=c5, kernel_size=3, padding=1, l_tau=l_tau, soft=soft, rec=self.rec, forget=self.forget, dual=self.dual, gpu=gpu)
